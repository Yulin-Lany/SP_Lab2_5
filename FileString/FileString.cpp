// Stseniaeva уч. гр. 20331 LB2-5 var 6
...
INT_PTR CALLBACK DbWndProc_About(HWND, UINT, WPARAM, LPARAM);
...
void WndProc_OnCommand(HWND hWnd, int id, HWND hwndCtrl, UINT codeNotify)
{
	...
	case CLOSE_HANDLE:
		if (!CloseProektToAddress())
			MessageBox(hWnd, TEXT("Ошибка закрытия!"), TEXT("ОШИБКА!"), MB_OK);

		else
		{
			MessageBox(hWnd, TEXT("Закрытие прошло успешно!\nНажмите ОК, чтобы увидеть результат"), TEXT("ОПОВЕЩЕНИЕ"), MB_OK);
...
INT_PTR CALLBACK DbWndProc_About(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
